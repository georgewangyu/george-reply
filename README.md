...
[![Stars](https://img.shields.io/github/stars/georgewangyu/george-reply?style=flat&color=black)](https://github.com/georgewangyu/george-reply/stargazers)
[![Built with Next.js](https://img.shields.io/badge/Next.js-16-black.svg)](https://nextjs.org)

</div>

George Reply is George Wang's personal fork of
[OpenReply](https://github.com/diwenne/openreply). It keeps the upstream MIT
license and attribution while adapting the product for a single-owner creator
workflow.

Someone comments `LINK` on your reel, and they get a DM with your link a second later. That is the whole idea. George Reply watches the comments on your Instagram posts, and when a comment matches a keyword you set, it sends that person a private reply through the official Meta API. You can also post a public reply under the comment at the same time.

ManyChat does this and charges a monthly fee. George Reply is the same core feature, free, running on your own infrastructure, with no seat limits and no plan caps.

> **George Reply is self-hosted. You have to deploy your own copy.**
>
...
George Reply is built around Meta's official Instagram private replies. It does not scrape, it does not automate a browser, and it never asks for an Instagram password. That keeps your account inside Meta's rules, which matters if you care about not getting flagged.
...
2. Meta sends a webhook to your George Reply instance.
3. George Reply checks the text against your active campaigns.
...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/georgewangyu/george-reply)

### Run it locally

```bash
git clone https://github.com/georgewangyu/george-reply.git
cd george-reply
npm install
...
For a compact, single-server AWS deployment, see
[docs/deploy-aws-ec2.md](docs/deploy-aws-ec2.md).

The optional always-on macOS runner at `scripts/run-local-service.sh` keeps its
private public hostname outside Git. Copy `.env.local-service.example` to
`.env.local-service`, set `LOCAL_SERVICE_PUBLIC_URL`, and keep the resulting
file local; `.env.local-service` is ignored by default.

Asset origin and license notes for repository-owned icons are recorded in
...
George Reply is a fork of OpenReply, built and maintained upstream by Diwen
Huang.
...
OpenReply was initially forked from
[instagram-comment-to-dm](https://github.com/im-anishraj/instagram-comment-to-dm)
by [Anish Raj](https://github.com/im-anishraj), also MIT licensed, and has been
substantially built upon since.

## Star the repo

If George Reply is useful to you, star it. It is the simplest way to help the project reach the next person looking for a free way to do this.
